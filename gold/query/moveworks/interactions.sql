-- ==============================================================================
-- Gold Mart Query: interactions.sql
-- Source System: moveworks
-- Hierarchy: s3://<bucket>/gold/query/moveworks/interactions.sql
-- Produces:
--   - Physical Table   : gold_tbl_interactions
--   - Presentation View: v_interactions
...
WITH conversation_topics AS (
    ...
    SELECT
        conversation_id,
        concat_ws(', ', collect_set(trim(detail_entity))) AS conversation_topic
    FROM
        tbl_interactions
    WHERE
        _is_current = 'Y'
...
bot_responses AS (
    ...
    SELECT
        ...
        first(detail_content, true) AS bot_response
    FROM
        tbl_interactions
    WHERE
        _is_current = 'Y'
...
plugin_aggregates AS (
    ...
    SELECT
        ...
        concat_ws(', ', collect_set(
            CASE WHEN (served = false OR served IS NULL) AND (used = false OR used IS NULL) THEN plugin_name END
        )) AS unsuccessful_plugins,
        concat_ws(', ', collect_set(
            CASE WHEN served = true THEN plugin_name END
        )) AS plugin_served,
        concat_ws(', ', collect_set(
            CASE WHEN served = true AND used = true THEN plugin_name END
        )) AS plugin_used
    FROM
        tbl_plugin_calls
    WHERE
...
resource_aggregates AS (
    ...
    SELECT
        interaction_id,
        concat_ws(', ', collect_set(
            CASE WHEN detail_domain IS NOT NULL AND trim(detail_domain) != '' THEN detail_domain END
        )) AS resource_domain,
        count(DISTINCT resource_id) AS no_of_citations,
        concat_ws(', ', collect_set(
            CASE WHEN detail_name IS NOT NULL AND trim(detail_name) != '' THEN detail_name END
        )) AS content_item_name,
        concat_ws(', ', collect_set(
            CASE WHEN detail_external_resource_id IS NOT NULL AND trim(detail_external_resource_id) != '' THEN detail_external_resource_id END
        )) AS content_item_id,
        max(CASE WHEN type = 'RESOURCE_TYPE_TICKET' THEN 'user initiated ticket' ELSE NULL END) AS ticket_type,
        concat_ws(', ', collect_set(
            CASE WHEN type = 'RESOURCE_TYPE_TICKET' AND detail_external_resource_id IS NOT NULL AND trim(detail_external_resource_id) != '' THEN detail_external_resource_id END
        )) AS ticket_id
    FROM
        tbl_plugin_resources
    WHERE
        _is_current = 'Y'
        AND _is_deleted = 'N'
        AND interaction_id IS NOT NULL
